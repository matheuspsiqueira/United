...
export default function VersiculosFavoritosModal({ visible, onClose }) {
  const { token } = useAuth();

  const [itens, setItens] = useState([]); // [{ verseId, cor, versao, livroSlug, capitulo, numero, texto, nomeLivro, carregando, erro }]
  ...
  const carregar = useCallback(async () => {
    ...
    setErroLista(null);
    try {
      const favoritos = await getGrifos(token); // [{ verse_id, cor }]

      const base = favoritos
        .map((f) => {
          const [versao, livroSlug, capitulo, numero] = f.verse_id.split('|');
          if (!versao || !livroSlug || !capitulo || !numero) return null; // ids antigos (formato com "-") não são recuperáveis
          return {
            verseId: f.verse_id,
            cor: f.cor,
            versao,
      ...
      // Busca o texto de cada versículo em paralelo
  ...
  const remover = async (verseId) => {
    setItens((atual) => atual.filter((i) => i.verseId !== verseId)); // otimista
    try {
      await removerGrifoApi(token, verseId);
    } catch (e) {
      carregar(); // falhou — recarrega do zero pra não ficar com estado errado
    }
  };
